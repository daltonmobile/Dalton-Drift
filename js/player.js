...
const Player = (() => {
  ...
  let smoke=[],sparks=[],steerLeft=false,steerRight=false,frameCount=0;
  ...
  function update(W,H,cam,speed){
    ...
    if(x<rL){x=rL;velX*=-0.15;} if(x>rR){x=rR;velX*=-0.15;}
    y=defY(H,cam);
    ...
    smoke=smoke.filter(p=>p.life>0); sparks=sparks.filter(p=>p.life>0);
  ...
  function draw(ctx, cam, nitroActive){
    ...
    // Nitro flames
    if(nitroActive) Sprites.drawNitroFlames(ctx,x,y,frameCount);

    const blink=invincibleFrames>0&&Math.floor(invincibleFrames/5)%2===0;
    if(!blink){
      // Clear any background bleed — draw with normal compositing only
      ctx.save();
      Sprites.drawPlayer(ctx,x,y,CAR_W*1.85,CAR_H*1.45,driftAngle,cam);
      ctx.restore();
    }
